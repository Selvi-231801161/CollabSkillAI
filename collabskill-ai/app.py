...
def render_navbar():
    ...
    # ── Actual Streamlit buttons — pulled up over navbar via CSS ──
    # Use one wide row with equal slim columns for every nav item
    total = len(nav_items)
    cols  = st.columns([2.8] + [1.05] * total)

    # Empty logo placeholder column
    with cols[0]:
    ...
    for col, (lbl, pg) in zip(cols[1:], nav_items):
        ...
        with col:
            if pg == "__logout__":
                st.markdown("<div class='cs-navbar btn-danger'>", unsafe_allow_html=True)
                ...
                st.markdown("<div class='cs-navbar cs-navbar-active'>", unsafe_allow_html=True)
                if st.button(display_lbl, key=f"nav__{pg}", use_container_width=True):
                    go(pg)
                st.markdown("</div>", unsafe_allow_html=True)
            else:
                st.markdown("<div class='cs-navbar'>", unsafe_allow_html=True)
                if st.button(display_lbl, key=f"nav__{pg}", use_container_width=True):
                    go(pg)
                st.markdown("</div>", unsafe_allow_html=True)

    # ── Pull button row UP to sit inside the navbar ───────────
    st.markdown("""
    <style>
    /* Lift the navbar button row to overlap the HTML navbar bar */
    ...
    /* Remove Streamlit's default column gaps in the nav row */
    section.main > div > div:nth-child(2) > div[data-testid="stHorizontalBlock"]
    ...
    /* Make ALL nav buttons transparent, borderless, compact */
    section.main > div > div:nth-child(2) > div[data-testid="stHorizontalBlock"]
        .stButton > button {
        background: transparent !important;
        color: #374151 !important;
...
def _render_interested_learners(t):
    ...
    for r in learners:
        ...
        st.markdown(
            ...
            f"font-size:14px;font-weight:700;color:#fff;flex-shrink:0;'>{ini}</div>"
            f"<div style='flex:1;'>"
            f"<div style='font-weight:700;color:#0F172A;font-size:13px;'>{r['learner_name']}</div>"
            f"<div style='font-size:11px;color:#64748B;'>{r.get('learner_skills', '')}</div>"
            f"<div style='font-size:11px;color:#94A3B8;'>{r.get('learner_exp', '')} "
...
def page_book_session():
    ...
    st.markdown(
        ...
        f"<div>"
        f"<div style='font-weight:800;color:#0F172A;font-size:15px;'>"
