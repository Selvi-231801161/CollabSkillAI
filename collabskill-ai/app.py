...
# ═══════════════════════════════════════════════════════════════
#  GLOBAL CSS  (keeps your original dark theme)
# ═══════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
/* ── Hero text (your original classes) ── */
...
.page-title { font-size:26px; font-weight:900; color:#f1f5f9; letter-spacing:-0.02em; margin-bottom:4px; }
...
.admin-only-banner {
    ...
    border-radius: 10px; padding: 10px 16px; color: #f87171;
...
# ═══════════════════════════════════════════════════════════════
def status_badge(s):
    c = {"open":"badge-green","in_progress":"badge-amber",
         "closed":"badge-slate","pending":"badge-amber",
         "accepted":"badge-green","rejected":"badge-red"}.get(s,"badge-slate")
...
def empty_state(icon, title, desc=""):
    st.markdown(f"""
    <div class='cs-card' style='text-align:center;padding:44px;'>
...
def render_navbar():
    ...
    if is_admin():
        ...
        with cols[0]:
            st.markdown(
                "<h3 style='color:#e5e7eb;padding-top:8px;margin:0;'>🚀 CollabSkill AI</h3>",
                unsafe_allow_html=True)
        with cols[1]:
            if st.button("Login"):    go("login")
        with cols[2]:
            if st.button("Sign Up"):  go("register")
        st.markdown("<hr style='border-color:#1e293b;margin:6px 0 18px;'/>",
                    unsafe_allow_html=True)
        return

    with cols[0]:
        admin_pill = (" <span style='font-size:10px;background:rgba(34,211,238,0.1);"
...
def page_landing():
    ...
    # ── Platform stats (public, aggregate only) ───────────────
    ...
    st.markdown(
        ...
        "margin-bottom:16px;'>WHAT WE OFFER</div>",
        unsafe_allow_html=True)

    f1, f2, f3, f4 = st.columns(4)
    feats = [
        ("🤖","rgba(34,211,238,.08)","AI Matching",
         "Smart AI finds the top 3 best-fit collaborators for any task instantly."),
        ("🛡️","rgba(124,58,237,.08)","Trust Scores",
         "Reputation scores from real peer feedback after every collaboration."),
        ("📋","rgba(34,197,94,.08)","Task Board",
         "Post tasks, browse open work, filter by skill and category."),
        ("🌐","rgba(245,158,11,.08)","Community",
         "Connect with developers, designers, marketers and creators."),
    ]
    for col, (icon, bg, title, desc) in zip([f1,f2,f3,f4], feats):
        col.markdown(f"""
        <div class='cs-card' style='text-align:center;min-height:170px;'>
            <div style='width:42px;height:42px;border-radius:11px;background:{bg};
                display:flex;align-items:center;justify-content:center;
                font-size:20px;margin:0 auto 12px;'>{icon}</div>
...
# ═══════════════════════════════════════════════════════════════
#  PAGE: LOGIN  (your original layout preserved)
# ═══════════════════════════════════════════════════════════════
def page_login():
    render_navbar()
    back_btn("← Home")
    ...
    with center:
        st.markdown("""
        <div style="background-color:#0f172a;padding:40px;
            border-radius:15px;box-shadow:0 0 30px rgba(0,0,0,0.5);">
        """, unsafe_allow_html=True)

        st.markdown(
            "<h2 style='color:#e5e7eb;text-align:center;'>Login</h2>",
            unsafe_allow_html=True)
        ...
        st.markdown(
            "<p style='color:#9ca3af;margin-top:12px;text-align:center;'>"
...
# ═══════════════════════════════════════════════════════════════
#  PAGE: REGISTER  (your original layout preserved)
# ═══════════════════════════════════════════════════════════════
def page_register():
    render_navbar()
    back_btn("← Home")
    ...
    st.markdown(
        "<h1 style='color:#e5e7eb;'>Create Your Profile</h1>",
        unsafe_allow_html=True)
    st.markdown(
        "<p style='color:#9ca3af;'>Join the skill exchange community</p>",
        unsafe_allow_html=True)
    ...
    if st.button("Create Account", use_container_width=True):
        if not agree:
            ...
            # Call matches your original signature:
    ...
    st.markdown(
        "<p style='color:#9ca3af;margin-top:10px;'>Already have an account?</p>",
        unsafe_allow_html=True)
    if st.button("Login"):
        go("login")
...
# ═══════════════════════════════════════════════════════════════
#  PAGE: USER DASHBOARD  — personal only, NO global analytics
# ═══════════════════════════════════════════════════════════════
def page_dashboard():
    require_login()
    if is_admin():
    ...
    st.markdown(
        f"<div class='page-title'>Hello, {u['username']}! 👋</div>",
        unsafe_allow_html=True)
    st.markdown(
        "<div class='page-sub'>Your personal workspace — only your tasks and activity</div>",
        unsafe_allow_html=True)
    ...
    with tab1:
        if not my_tasks:
            empty_state("📭", "No tasks yet",
                        "Post your first task to start finding collaborators.")
    ...
    with tab2:
        if not my_apps:
            empty_state("📩", "No applications yet",
                        "Browse tasks and apply to help others.")
...
def page_browse_tasks():
    ...
    with fc1: search   = st.text_input("", placeholder="🔍 Search…", label_visibility="collapsed")
    with fc2: category = st.selectbox("", ["All"] + CATEGORIES, label_visibility="collapsed")
    with fc3: sort_by  = st.selectbox("", ["newest", "oldest", "priority"],
    ...
    for t in tasks:
        with st.expander(f"📌  {t['title']}  —  by {t['creator_name']}  [{t['category']}]"):
            ...
            with c1:
                st.markdown(f"""
                <div style='color:#94a3b8;font-size:13px;line-height:1.7;margin-bottom:8px;'>{t['description']}</div>
                {status_badge(t['status'])} {priority_badge(t['priority'])}
                <span class='cs-badge badge-violet'>{t['category']}</span>
                <span class='cs-badge badge-cyan'>🛠 {t['skills']}</span>
    ...
    with c_form:
        with st.form("post_task_form"):
            title       = st.text_input("Task Title *",
                                        placeholder="e.g. Need a React developer for a dashboard")
            description = st.text_area("Description *",
                                       placeholder="Describe the task in detail…", height=130)
            ...
            if st.form_submit_button("🚀 Post Task", use_container_width=True):
                if not all([title, description, skills]):
                    ...
                    create_task(title, description, skills, category,
                                deadline, priority, st.session_state.user["id"])
    ...
    with c_tip:
        ...
        if recent:
            st.markdown(
                ...
                "margin-bottom:8px;'>📌 My Recent Posts</div>",
                unsafe_allow_html=True)
            for t in recent:
                dot = "🟢" if t["status"] == "open" else "🔴"
                st.markdown(
                    f"<div style='color:#64748b;font-size:12px;padding:5px 0;"
                    f"border-bottom:1px solid #1e293b;'>{dot} {t['title']}</div>",
                    unsafe_allow_html=True)
...
def page_profile():
    ...
    with sidebar:
        ...
        skills_html = "".join(
            ...
            for s in (u["skills"] or "").split(",") if s.strip()
        ) or "<span style='color:#475569;font-size:12px;'>No skills added</span>"

        st.markdown(f"""
        <div class='cs-card' style='text-align:center;'>
            <div style='margin:0 auto 14px;display:flex;justify-content:center;'>
                {avatar_html(u['username'], 68)}
            </div>
            ...
            <div style='margin:8px 0;'>
                <span class='cs-badge badge-violet'>{u['experience']}</span>
                {'<span class="cs-badge badge-cyan">Admin</span>' if u["role"]=="admin" else ''}
            </div>
            <div style='color:#94a3b8;font-size:13px;line-height:1.6;margin-top:8px;'>{u['bio'] or 'No bio added.'}</div>
            {'<div style="margin-top:10px;"><a href="' + u["portfolio"] + '" target="_blank" style="color:#22d3ee;font-size:13px;">🔗 Portfolio</a></div>' if u.get("portfolio") else ''}
            <div style='margin-top:14px;'>
                ...
                <div style='color:#475569;font-size:11px;margin-top:4px;'>{u['total_ratings']} ratings</div>
            </div>
        </div>
        <div class='cs-card' style='margin-top:0;'>
            <div style='color:#94a3b8;font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;'>SKILLS</div>
            {skills_html}
        </div>
        """, unsafe_allow_html=True)
    ...
    with main:
            ...
            with st.form("edit_profile"):
                ...
                n_exp  = st.selectbox(
                    ...
                    index=["Beginner","Intermediate","Advanced","Expert"].index(u["experience"])
                          if u["experience"] in ["Beginner","Intermediate","Advanced","Expert"]
                          else 0)
        ...
        with t3:
            ...
            if not fbs:
                ...
                st.markdown(f"""
                <div class='cs-card' style='text-align:center;padding:18px;margin-bottom:12px;'>
        ...
        with t4:
            ...
            if not others:
                ...
                chosen  = st.selectbox("Select a collaborator to rate", opts)
                ...
                if st.button("Submit Feedback ✅", use_container_width=True, key="fb_submit"):
...
def page_ai_match():
    ...
    with left:
        with st.form("ai_form"):
            title  = st.text_input("Task Title",    placeholder="e.g. Build a data dashboard")
            desc   = st.text_area("Description",    placeholder="Describe what you need…", height=110)
                ...
                with st.spinner("🤖 AI is analyzing all profiles…"):
                    try:
                        ...
                        matches = match_users_to_task(
                            title, desc, skills, st.session_state.user["id"])
    ...
    if st.session_state.ai_done:
        ...
        if matches:
            st.markdown(
                f"<br><div style='color:#22d3ee;font-weight:700;font-size:17px;"
            ...
            for i, m in enumerate(matches, 1):
                ...
                with mc1:
                    st.markdown(f"""
                    <div style='background:linear-gradient(135deg,#0f172a,#130f2a);
                ...
                if row:
                    if st.button(f"📩 Notify {m['name']}", key=f"ntfy_{i}"):
...
def page_community():
    ...
    with sc1: search = st.text_input("", placeholder="🔍 Search members…",
                                     label_visibility="collapsed")
    with sc2: exp_f  = st.selectbox("", ["All Levels","Beginner","Intermediate","Advanced","Expert"],
                                    label_visibility="collapsed")

    where, params = ["role='user'","is_active=1",f"id!='{st.session_state.user['id']}'"], []
    if search:
    ...
    st.markdown(
        ...
        f"{len(users)} member(s)</div>", unsafe_allow_html=True)
    ...
    for i in range(0, len(users), 3):
        ...
        for col, u in zip(cols, users[i:i+3]):
            ...
            <div class='cs-card'>
                ...
                <div>{''.join([f"<span class='cs-badge badge-cyan' style='font-size:10px;'>{s.strip()}</span>" for s in (u['skills'] or '').split(',')[:3] if s.strip()])}</div>
                {trust_bar(u['trust_score'])}
                <div style='color:#334155;font-size:10px;margin-top:3px;'>{u['total_ratings']} ratings</div>
                {'<div style="margin-top:8px;"><a href="' + u["portfolio"] + '" target="_blank" style="color:#22d3ee;font-size:12px;">🔗 Portfolio</a></div>' if u.get("portfolio") else ''}
            </div>""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
#  PAGE: ADMIN DASHBOARD  (platform-wide — admin only)
# ═══════════════════════════════════════════════════════════════
def page_admin_dashboard():
    ...
    <div class='admin-only-banner'>
        🛡️ ADMIN ONLY — This page and its data are NOT visible to regular users.
    </div>""", unsafe_allow_html=True)

    st.markdown("<div class='page-title'>📊 Admin Dashboard</div>", unsafe_allow_html=True)
    st.markdown("<div class='page-sub'>Full platform analytics — visible only to you</div>",
                unsafe_allow_html=True)
    ...
    with col2:
        ...
        rec_u = db_fetchall(
            "SELECT username AS name,'Joined' AS action,created_at FROM users "
            "ORDER BY created_at DESC LIMIT 5")
        rec_t = db_fetchall(
            "SELECT title AS name,'Task posted' AS action,created_at FROM tasks "
            "ORDER BY created_at DESC LIMIT 5")
        ...
        for a in activity:
            ...
            <div style='display:flex;align-items:flex-start;gap:10px;
                ...
                <div style='width:7px;height:7px;border-radius:50%;
                    background:{dot_c};margin-top:5px;flex-shrink:0;'></div>
                <div>
                    <div style='color:#e2e8f0;font-size:13px;'>
                        {a['action']}: <b>{a['name']}</b></div>
...
def page_admin_users():
    ...
    sc1, sc2 = st.columns([3, 1])
    with sc1: search = st.text_input("", placeholder="🔍 Search…", label_visibility="collapsed")
    ...
    st.markdown(
        f"<div style='color:#475569;font-size:13px;margin-bottom:14px;'>"
        f"{len(users)} user(s)</div>", unsafe_allow_html=True)

    for u in users:
        with st.expander(
            f"👤 {u['username']}  [{u['role'].upper()}]  —  {u['email']}"):
...
def page_admin_tasks():
    ...
    with sc1: search   = st.text_input("", placeholder="🔍 Search…", label_visibility="collapsed")
    with sc2: status_f = st.selectbox("", ["All","open","closed","in_progress"],
                                      label_visibility="collapsed")
    ...
    st.markdown(
        f"<div style='color:#475569;font-size:13px;margin-bottom:14px;'>"
        f"{len(tasks)} task(s)</div>", unsafe_allow_html=True)
    ...
    for t in tasks:
        with st.expander(
            f"📌 {t['title']}  [{t['status'].upper()}]  —  {t['creator_name']}"):
            ...
            with tc2:
                new_s = st.selectbox(
                    ...
                    key=f"adm_s_{t['id']}")
                if st.button("Update", key=f"adm_u_{t['id']}"):
                    update_task_status(t["id"], new_s)
                    st.success("Updated.")
                    st.rerun()
                if st.button("🗑 Delete", key=f"adm_d_{t['id']}"):
                    delete_task(t["id"])
                    st.success("Deleted.")
                    st.rerun()
